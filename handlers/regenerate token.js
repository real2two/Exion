...
module.exports.load = async function(app) {
    app.post("/regen", async (req, res) => {
        ...
        await process.db.tokens_to_ids.delete(userinfo.token);

        userinfo.token = await functions.makeid(100);

        await process.db.user_info.set(id, userinfo);
        await process.db.tokens_to_ids.set(userinfo.token, id);

        return res.json({ error: "There are no errors.", id: id, token: userinfo.token });
    });
};